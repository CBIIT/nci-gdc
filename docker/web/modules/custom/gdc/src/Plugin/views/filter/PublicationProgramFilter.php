...
/**
 * Filter to display publication programs in a dropdown.
 *
 * @ingroup views_filter_handlers
 *
 * @ViewsFilter("publication_program_filter")
 */
class PublicationProgramFilter extends InOperator
{
    /**
     * {@inheritdoc}
     */
     ...
     public function query()
     {
         ...
         $this->query->addWhereExpression(
             $this->options["group"],
             "$table_alias.field_pub_cat_tags_target_id IN ($tags)"
         );
     }
    ...
    {
        $query =
        "SELECT DISTINCT(a.field_pub_cat_tags_target_id),c.name
                FROM node__field_pub_cat_tags a,
					 node_field_data b,
					taxonomy_term_field_data c
				where a.entity_id=b.nid and b.status=1 and a.field_pub_cat_tags_target_id = c.tid
                order by c.name";
        ...
        foreach ($result as $row) {
            $id = $row->field_pub_cat_tags_target_id;
            $name = $row->name;
    ...
    public function buildExposeForm(&$form, FormStateInterface $form_state) {
        ...
        $form['expose']['identifier'] = [
            '#type' => 'textfield',
            '#default_value' => 'field_pub_cat_tags_target_id',
